...
In this lab, we'll load a data set that we've used before, the airnb data, and make a map of airbnb prices in Chicago. Call it `airbnb`
...
## 3.1 Mapping AirBnb
The airbnb data already has latitude and longitude data for each advertised unit. So it's easy to put use the `addMarkers` function to add a marker for each listed unit in a sample of 10 units:
...
## 3.2 Saving a leaflet plot
You might want to save your leaflet plot. There are two options: saving and image and saving an interactive widget. You'll need the `mapview`, `webshot` and `htmlwidgets` packages. After installing `webshot`, you may need to run `webshot::install_phantomjs()`.

```{r}
library(mapview)
library(htmlwidgets)

# save an image with 'mapshot'
mapshot(m, file = "plots/example_map.png")

# save an html widget:
saveWidget(m, 'plots/example_map.html')
...
Modify the code above so that the price of the unit appears when you click on each marker. Save your map as an interactive widget.

Bonus: Modify the code so that the Title AND price of the unit appears when you click on each marker. Save your map as an image.
...
## 4.5 Making a Static Plot

Leaflet plots are nice for interactivity, but you might want to make a static plot too. 
...
chi_nhs %>%
  ...
  theme_void()
```
These can be much faster to render and save than leaflet plots.
